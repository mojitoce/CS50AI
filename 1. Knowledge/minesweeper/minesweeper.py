...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        p_sentence = Sentence(nearby_cells, count)

        print('Safes', self.safes - self.moves_made)
        print('Mines', self.mines)
        print('Pot Sentence', p_sentence.__str__())

        self.knowledge.append(p_sentence)

        # Run loop as long as:
        # - There is a sentence where we can determine mines or safes
        # - There is a sentence that is subset of another
        while True:
            for s1 in self.knowledge:
                if len(s1.cells) == 0:
                    self.knowledge.remove(s1)
                    continue
                elif not self.check_add(s1):
                    continue
                for s2 in self.knowledge:
                    if len(s2.cells) == 0:
                        self.knowledge.remove(s2)
                        continue
                    if not self.check_add(s2):
                        continue
                    if (s1.cells.issubset(s2.cells)) and (not s1.__eq__(s2)):
                        ...
                        continue
            break
        print('Safes after', self.safes - self.moves_made)
            ...
            for j in range(self.width):
                if ((i,j) not in self.moves_made) and ((i,j) not in self.mines):
                    if ((i,j) in self.safes):
                        print('safe in random chosen', (i,j))
    ...
    def get_nearby_cells(self, cell):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if ((i,j) in self.safes or (i,j) in self.mines or (i,j) in self.moves_made):
                        pass
                    else:
                        cells.add((i, j))
        return cells
